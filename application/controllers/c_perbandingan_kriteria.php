<?php
	class C_perbandingan_kriteria extends CI_Controller {
		function __construct(){
			parent::__construct();
			$this->load->model("m_perbandingan_kriteria");
        }
        ...
        $data['perbandingan_kriteria'] = $this->m_perbandingan_kriteria->select_perbandingan_kriteria()->result();     
        ...
        foreach ($data['daftar_kriteria'] as $kriteria) {
            ...
            $data2['bobot'] = $bobot;
            $this->m_perbandingan_kriteria->update_bobot_kriteria($kriteria->id_kriteria, $data2);
            $index4++;
        }
        //============================================================================
        ...
        $this->load->view('administrator/v_data_perbandingan_kriteria', $data);
    ...
    public function edit_perbandingan_kriteria(){
        $this->load->view('administrator/element/header');
        $this->load->view('administrator/element/navigation');
        ...
        $this->load->view('administrator/v_edit_data_perbandingan_kriteria',$data);
        $this->load->view('administrator/element/footer');
    }
    
    public function update_perbandingan_kriteria(){ 
        
        // mengupdate bobot relasi
        // i mewakili id kriteria 1
        // j mewakili id kriteria 2
        $id_perbandingan_kriteria = 0;
        for ($h=1; $h <= 36 ; $h++) { 
            $id_perbandingan_kriteria = $id_perbandingan_kriteria + 1;
            for($i = 1; $i <= 6; $i++){
            for($j = 1; $j <= 6; $j++){
                $data['bobot'] = $this->input->post(''.$i.$j);
                $this->m_perbandingan_kriteria->update_bobot_perbandingan_kriteria($id_perbandingan_kriteria, $i, $j, $data);
            }          
        }
        }
        redirect('C_perbandingan_kriteria/perbandingan_kriteria');
   }    
}