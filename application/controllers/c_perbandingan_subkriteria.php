<?php
    class C_perbandingan_subkriteria extends CI_Controller {
        function __construct(){
            parent::__construct();
            $this->load->model("m_perbandingan_subkriteria");
        }
        ...
        $data['daftar_subkriteria'] = $this->m_perbandingan_subkriteria->select_subkriteria($id)->result();
        $data['perbandingan_subkriteria'] = $this->m_perbandingan_subkriteria->select_perbandingan_subkriteria($id)->result();
        ...
        foreach ($data['daftar_subkriteria'] as $subkriteria) {
            ...
            $data2['bobot'] = $bobot;
            $this->m_perbandingan_subkriteria->update_bobot_subkriteria($id, $subkriteria->id_subkriteria, $data2);
            $index4++;
        }
        //============================================================================
        ...
        $this->load->view('administrator/v_data_perbandingan_subkriteria', $data);
    ...
    public function edit_perbandingan_subkriteria($id_topik_skripsi){
        $this->load->view('administrator/element/header');
        $this->load->view('administrator/element/navigation');

        $data['daftar_subkriteria'] = $this->m_perbandingan_subkriteria->select_subkriteria($id_topik_skripsi)->result();
        $data['jumlah'] = count($data['daftar_subkriteria']);
        $data['id'] = $id_topik_skripsi;

        $this->load->view('administrator/v_edit_data_perbandingan_subkriteria',$data);
        $this->load->view('administrator/element/footer');
    }
    
    public function update_perbandingan_subkriteria($id_topik_skripsi){ 
        // mengupdate bobot relasi
        // i mewakili id subkriteria 1
        // j mewakili id subkriteria 2
        
        //if ($id_topik_skripsi == 1) {
           for($i = 1; $i <= 4; $i++){
            for($j = 1; $j <= 4; $j++){
                $data['bobot'] = $this->input->post(''.$i.$j);
                $this->m_perbandingan_subkriteria->update_bobot_perbandingan_subkriteria($id_topik_skripsi, $i, $j, $data);
            }          
            }
        // }
        // else{
        ...
        //         $l=5;
        //         $id_topik_skripsi = 2;
        //         $data['bobot'] = $this->input->post(''.$k.$l);
        //         $this->m_perbandingan_subkriteria->update_bobot_perbandingan_subkriteria($id_topik_skripsi, $k, $l, $data);
        //         $l++;
        //     }
        //         $k++;          
        // }
        ...
        // }
        redirect('C_perbandingan_subkriteria/perbandingan_subkriteria/'.$id_topik_skripsi);
        }    
}